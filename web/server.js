...
function handle(req, response, session) {
  ...
  if (url.pathname === '/engine-test') { const filePath = path.join(root, 'public', 'engine-test.html'); if (!fs.existsSync(filePath)) return json(response, 404, {error:'not found'}); response.writeHead(200, {'Content-Type':files['.html']}); return fs.createReadStream(filePath).pipe(response); }
  if (url.pathname === '/test-data/wasm-parity-100.fen') { const filePath = path.resolve(root, '../tests/data/wasm-parity-100.fen'); if (!fs.existsSync(filePath)) return json(response, 404, {error:'not found'}); response.writeHead(200, {'Content-Type':'text/plain; charset=utf-8'}); return fs.createReadStream(filePath).pipe(response); }
