...
test('two owned games are isolated and enforce revision and engine trust boundary',async()=>{
  ...
  let moveA=await call('/api/move',{gameId:gameA,revision:0,move:'e2e4'},a), moveB=await call('/api/move',{gameId:gameB,revision:0,move:'d2d4'},b);
  ...
  assert.equal((await call('/api/engine-move',{gameId:gameA,revision:1,move:'e7e5'},b)).status,403);
  assert.equal((await call('/api/engine-move',{gameId:gameA,revision:1,move:'e7e6'},a)).status,200);
...
test('black-owned game starts on the engine turn without a native process',async()=>{
  ...
  assert.equal((await call('/api/engine-move',{gameId:started.data.gameId,revision:0,move:'e2e4'},owner)).status,200);
